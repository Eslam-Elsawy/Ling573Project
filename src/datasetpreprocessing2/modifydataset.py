...
def main():
    directoryPath = getDirectoryPath("../../input/documents2/")
    evaldatasetfiles = getFilePath("evaldatasetfiles")
    with io.open(evaldatasetfiles, 'r', encoding='utf8') as evalFiles:
        for line in evalFiles:

            filename = line.strip()
            input_file_path = directoryPath + "/" + filename
            output_file_path = directoryPath + "/" + filename + ".xml"
            print("Modifying: " +input_file_path)
